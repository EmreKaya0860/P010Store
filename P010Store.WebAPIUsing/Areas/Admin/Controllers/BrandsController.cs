...
namespace P010Store.WebAPIUsing.Areas.Admin.Controllers
{
    ...
    public class BrandsController : Controller
    {
        ...
        private readonly string _apiAdres = "https://localhost:7083/api/Brands";
        ...
        {
            var model = await _httpClient.GetFromJsonAsync<List<Brand>>(_apiAdres); // _httpClient nesnesi içerisindeki GetFromJsonAsync metodu _apiAdres adres içerisine yazdığımız
            //api ye get isteği atıp oradan dönen json listesini bizim belirttiğimiz List<Brand> marka listesine dönüştürür.
        ...
        public async Task<ActionResult> Create(Brand brand, IFormFile? Logo)
        {
            if (ModelState.IsValid) // Model class ımız olan brand nesnesinin validasyonu için koyduğumuz kurallara (örneğin marka adı required-boş geçilemez gibi) uyulmuşsa
            {
                try
                {
                    ...
                    var response = await _httpClient.PostAsJsonAsync(_apiAdres, brand); // _httpClient nesnesi içerisindeki PostAsJsonAsync metoduna parametre olarak post isteği atacağımız
                    // adresi(_apiAdres) ve eklemek istediğimiz nesneyi(brand) verdiğimizde brand i json a çevirerek api ye eklenmek üzere yolluyor. Eğer bir eksik yoksa api brandi
                    //ekler ve geriye response nesnesine bir cevap ekler.
