namespace P010Store.WebAPIUsing.Utils
{
    public class FileHelper
    {
        ...
        public static bool FileRemover(string fileName, string filePath = "/wwwroot/Img/")
        {
            ...
            if (File.Exists(directory)) // .net de dosya ve klasör işlemleri için File sınıfı mevcuttur. File.Exists metodu kendisine verilen yolda(directory) bir dosya var mı yok
                                        // yok mu kontrol eder. Dosya varsa true yoksa false bilgisini getirir.
            {
                File.Delete(directory); // Eğer klasörde dosya varsa dosyayı fiziksel oalrak sil.
