...
namespace P010Store.WebUI.Areas.Admin.Controllers
{
    ...
    public class ProductsController : Controller
    {
        ...
        private readonly IProductService _productService; // InvalidOperationException: Unable to resolve service for type 'P010Store.Service.Abstract.IProductService' while attempting to activate 'P010Store.WebUI.Areas.Admin.Controllers.ProductsController'.
        //Bu servisi kullandığımızda bu hatayı alırız, bu sorunu çözmek için servisi program.cs de tanımlamamız gerekir.
        ...
        {
            var model = await _productService.GetAllProductsByCategoriesBrandsAsync(); //eskisi // await _service.GetAllAsync();
            return View(model);
        }
        ...
        public async Task<ActionResult> CreateAsync(Product product, IFormFile? Image)
        {
            ...
            //Burada ürün ekleme esnasında ekleme başarısız olur ise ekrandaki select elementlerine verileri tekrar gönderiyoruz aksi taktirde null reference hatası alırız
            ViewBag.CategoryId = new SelectList(await _serviceCategory.GetAllAsync(), "Id", "Name");
            ViewBag.BrandId = new SelectList(await _serviceBrand.GetAllAsync(), "Id", "Name");
            return View(product);
        }
